...
//ConvertXY=>Hex (uint32 = bytes4)
function convertXYtoHexUint32(x, y) {
    var hexX = window.tronWeb.toHex(Math.abs(x)).substr(2);
    var hexY = window.tronWeb.toHex(Math.abs(y)).substr(2);

    //@dev: Uint32 is need 4 bytes for storage value, 4byte present 8 letter hex string (not include 0x)
    var hexReturn = "0x" + (new Array(4 - hexX.length + 1)).join(0) + hexX + (new Array(4 - hexY.length + 1)).join(0) + hexY
    return hexReturn
}
function hexUint32toXY(hexInput) {
    //@dev: Uint32 is need 4 bytes for storage value, 4byte present 8 letter hex string (not include 0x)
    let hex = (new Array(8 + 1 - (hexInput.length - 2))).join(0);
    hex = hex + hexInput.substr(2);
    let hexY = hex.substr(hex.length - 4, 4);
    let hexX = hex.substr(hex.length - 8, 4);
    if (hexX == "") hexX = "0";
    if (hexY == "") hexY = "0";
    let y = window.tronWeb.toDecimal("0x" + hexY) ;
    let x = window.tronWeb.toDecimal("0x" + hexX) ;
    return { x, y }
}