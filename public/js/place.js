...
$(document).ready(() => {
  ...
  //pixel colors 
  var pixelColors = ["#000000","#ffffff","#e4e4e4","#888888","#222222","#a06a42","#e50000","#e59500","#e5d900","#94e044","#02be01","#00d3dd","#0083c7","#0000ea","#820080","#cf6ee4","#ffa7d1"];
  ...
  function handleMouseClick(event) {
  	...
  	if(oldPixels.length>=225){
  		showModalError(
  			"uh-oh..",
  			"You can buy maximum 225 pixels at once!<br>Lets buy to add more pixels!",	
  			""
  			);
  		return false;
  	}
    ...
    var colorIndex = $.inArray(rgbColor, pixelColors);
  	oldPixels.push({
  ...
  function handleMouseMove(event) {
  	...
  	if (!dragEnable) {
          ...
          let tempColor = global.canvasData[col][row];          
          var colorIndex = $.inArray(tempColor, pixelColors);
          let tempPosition = new Uint16Array(3);
          tempPosition.set([pixelX, pixelY,colorIndex]);
  ...
  $(".btn_buy").click(async function(event) {
    ...
    $("#buy-pixel").css("display", "none");
    $("#buy-spiner").css("display", "block");

    var test = await TRON.usertoCommunity();
    if (isEmpty(test) || hex2a(test) == "") {
    	$("#buy-pixel").css("display", "block");
    	$("#buy-spiner").css("display", "none");
